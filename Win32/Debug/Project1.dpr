...
begin
    ...
    begin
    writeln('Kembali ke A +',B[0]*10 );
    jarak:=jarak+B[0]*10;
    end
    else if(jalur[Length(jalur)-1]='C') then
    begin
    writeln('Kembali ke A +',C[0]*10 );
    jarak:=jarak+C[0]*10;
    end
    else if(jalur[Length(jalur)-1]='D') then
    begin
    writeln('Kembali ke A +',D[0]*10 );
    jarak:=jarak+D[0]*10;
    end
    else if(jalur[Length(jalur)-1]='E') then
    begin
    writeln('Kembali ke A +',E[0]*10 );
    jarak:=jarak+E[0]*10;
    end
    else if(jalur[Length(jalur)-1]='F') then
    begin
    writeln('Kembali ke A +',F[0]*10 );
    jarak:=jarak+F[0]*10;
    end
    else if(jalur[Length(jalur)-1]='G') then
    begin
    writeln('Kembali ke A +',G[0]*10 );
    jarak:=jarak+G[0]*10;
    end;
    Insert('A',jalur,Length(jalur));
    writeln('Rute Greedy');
    ...
    writeln('');
    writeln(jarak);
    writeln('Waktu total = ',floor(jarak/60),' Jam ',jarak mod 60,' Menit');
    readln(kota);
end.
